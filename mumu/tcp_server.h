 ...
 * @date 2019-06-05
 * @copyright Copyright (c) 2019年 muhui.yin All rights reserved (www.muhui.top)
 */
#ifndef __MUHUI_TCP_SERVER_H__
#define __MUHUI_TCP_SERVER_H__
...
class LexcalCast<std::string, TcpServerConf> {
  public:
    TcpServerConf operator()(const std::string& v) {
        ...
        conf.args =
            LexcalCast<std::string, std::map<std::string, std::string>>()(
...
class LexcalCast<TcpServerConf, std::string> {
  public:
    std::string operator()(const TcpServerConf& conf) {
        ...
        node["args"] = YAML::Load(
            LexcalCast<std::map<std::string, std::string>, std::string>()(
                conf.args));
...
class TcpServer : public std::enable_shared_from_this<TcpServer>, Noncopyable {
  public:
     ...
     */
    TcpServer(muhui::IOManager* worker = muhui::IOManager::GetThis(),
              muhui::IOManager* io_woker = muhui::IOManager::GetThis(),
              muhui::IOManager* accept_worker = muhui::IOManager::GetThis());
     ...
     * @return 返回是否绑定成功
     */
    virtual bool bind(muhui::Address::ptr addr, bool ssl = false);
    ...
    // bool loadCertificates(const std::string& cert_file, const std::string&
    // key_file);
